...
Assertion carrying the original source_id and verification, and Evidence for the triple. The
old ids and graph mappings are preserved (published_episodes and the graph maps are left in
place). The conversion is idempotent: re-running converges (fact_key dedups the Fact and the
assertion source key dedups the Assertion), so a partial run can be resumed. Free-text
episodes with no structured triple cannot be turned into a Fact with confidence and are
counted for review rather than given invented provenance (ADR-0006). See docs/runbooks.
...
class FabricBackfillService:
    ...
    async def backfill_group(self, *, group_id: str) -> BackfillReport:
        ...
        for row in rows:
            ...
            triples = row["payload"].get("triples") if isinstance(row["payload"], dict) else None
            if not triples:
    ...
    async def _backfill_triple(
        ...
        fk = fact_key(
            scope=group_id, subject_entity_id=entity.id, predicate=predicate, object_scalar=obj
        )
        lifecycle = (
        ...
        if existing is None:
            fact = await self._facts.upsert(
                Fact(
                    ...
                    normalized_object=normalize_object(object_scalar=obj),
                    object_scalar=obj,
